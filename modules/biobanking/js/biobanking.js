...
function checkBiobankingForm() {
    'use strict';
    var form = document.biobankingForm;
    ...
    $.get("AjaxHelper.php?Module=biobanking&script=validateProfileIDs.php&candID=" + form.candID.value + "&PSCID=" + form.PSCID.value ,
        function(data)
        {
            //ids are valid, submit accessProfileForm form
            if (data==1) {
                $( "#biobankingForm" ).unbind('submit.formValidation');
                form.submit();
            }
            else {
...
$(function(){
	$('input[name=date_collected]').datepicker({
		dateFormat: 'yy-mm-dd',
		changeMonth: true,
		changeYear: true
	});
...
$(document).ready(function() {
    ...
    $(".biospecimenLink").click(function(e) {
        var form = $('<form />', {
            "action" : "main.php?test_name=biobanking",
            "method" : "post"
        });
        var values = {
            "reset" : "true",
    ...
    $( "#biobankingForm" ).bind('submit.formValidation', function( event ) {
